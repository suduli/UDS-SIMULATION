...
export const SequenceExecutionPanel: React.FC<SequenceExecutionPanelProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-gradient-to-br from-gray-900 via-blue-900/20 to-gray-900 border-2 border-cyan-500/30 rounded-xl max-w-5xl w-full max-h-[90vh] overflow-hidden shadow-2xl shadow-cyan-500/20">
        {/* Header */}
          ...
          {!currentSequence ? (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm text-gray-400 mb-2">Error Handling</label>
                  <select
              ...
              {sequence && (
                <div className="p-4 bg-gray-800/30 rounded-lg border border-cyan-500/20">
                  <h4 className="text-white font-semibold mb-3">Sequence Preview</h4>
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {sequence.steps.map((step, index) => (
                      <div key={step.id} className="flex items-center gap-3 text-sm">
                        <span className="flex items-center justify-center w-6 h-6 rounded-full bg-cyan-500/20 text-cyan-400 text-xs">
                          {index + 1}
                        </span>
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-3">
                {currentSequence.isRunning && !currentSequence.isPaused && (
                  <button
                    onClick={handlePause}
                    className="flex-1 px-4 py-3 bg-yellow-500/20 border border-yellow-500/50 text-yellow-400 rounded-lg hover:bg-yellow-500/30 transition-colors flex items-center justify-center gap-2"
                  >
                ...
                {currentSequence.isPaused && (
                  <button
                    ...
                    className="flex-1 px-4 py-3 bg-green-500/20 border border-green-500/50 text-green-400 rounded-lg hover:bg-green-500/30 transition-colors flex items-center justify-center gap-2"
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                ...
                {currentSequence.isRunning ? (
                  <button
                    onClick={handleStop}
                    className="flex-1 px-4 py-3 bg-red-500/20 border border-red-500/50 text-red-400 rounded-lg hover:bg-red-500/30 transition-colors flex items-center justify-center gap-2"
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <button
                    onClick={handleClose}
                    className="flex-1 px-4 py-3 bg-cyan-500/20 border border-cyan-500/50 text-cyan-400 rounded-lg hover:bg-cyan-500/30 transition-colors flex items-center justify-center gap-2"
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
              ...
              {/* Results */}
              <div className="p-4 bg-gray-800/30 rounded-lg border border-cyan-500/20">
                <h4 className="text-white font-semibold mb-3">Execution Log</h4>
                <div className="space-y-2 max-h-64 overflow-y-auto">
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-white font-semibold">{result.step.label}</span>
                          {result.success ? (
