      ...
      <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
        {requestHistory.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            ...
            <p>No messages yet. Send a request to see the response.</p>
          </div>
        ) : (
              ...
              <div className="p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-gray-500 font-mono">
                    {((item.response.timestamp - item.request.timestamp))}ms
                  </span>
                </div>
                ...
                <div className={`rounded p-3 ${
                  ...
                  <div className={`font-mono text-sm ${item.response.isNegative ? 'text-cyber-pink' : 'text-cyber-green'}`}>
                  ...
                  <div className="mt-3 space-y-1 text-xs">
                    {item.response.data.map((byte, byteIdx) => (
                      <div key={byteIdx} className="flex items-center space-x-2 text-gray-400">
                        ...
                        <span className="text-gray-500">
                          {byteIdx === 0 && item.response.isNegative && '(NRC Response)'}
                          {byteIdx === 0 && !item.response.isNegative && '(Positive Response)'}
                          {byteIdx === 1 && item.response.isNegative && '(Service ID)'}
                          {byteIdx === 2 && item.response.isNegative && item.response.nrc && `(${getNRCDescription(item.response.nrc)})`}
                        </span>
                      </div>
                    ))}
                  </div>
                  ...
                  {item.response.data.length > 3 && !item.response.isNegative && (
                    <div className="mt-3 pt-3 border-t border-gray-700">
                      <div className="text-xs text-gray-400 mb-1">ASCII:</div>
                ...
                {item.response.isNegative && item.response.nrc && (
                  <div className="mt-3 p-3 bg-cyber-pink/10 border border-cyber-pink/30 rounded">
                    <div className="flex items-start space-x-2">
                      <svg className="w-5 h-5 text-cyber-pink flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
